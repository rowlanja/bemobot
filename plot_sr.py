...
class Supres(Values):
    ...
    def main(ticker_csv, selected_timeframe, candle_count=254):
        ...
        def plot_approaching_sr(sensitivty: float = 0.5):
            ...
            resistance_levels = []
            for (key, resistance_level) in resistance_list :
                print(resistance_list)
                if close > resistance_level and close < resistance_bound:
                    resistance_levels.append((key, resistance_level)) 
            support_levels = []
            for (key, support_level) in support_list :
                print(support_list)
                if close < support_level and close > support_bound :
                    support_levels.append((key, support_level)) 

            # Resistance lines
            for r in range(len(resistance_levels)):
                fig.add_shape(
                    type="line",
                    x0=resistance_levels[r][0] - 1,
                    y0=resistance_levels[r][1],
                    x1=len(df) + 25,
                    y1=resistance_levels[r][1],
                    line=dict(color="red", width=1),
                )
                # Resistance annotations
                fig.add_annotation(
                    x=len(df) + 20,
                    y=resistance_levels[r][1],
                    text="urgent - " + str(resistance_levels[r][1]),
                    font=dict(size=15, color="red"),
                )

            # Resistance lines
            for r in range(len(support_levels)):
                fig.add_shape(
                    type="line",
                    x0=support_levels[r][0] - 1,
                    y0=support_levels[r][1],
                    x1=len(df) + 25,
                    y1=support_levels[r][1],
                    line=dict(color="red", width=1),
                )
                # Resistance annotations
                fig.add_annotation(
                    x=len(df) + 20,
                    y=support_levels[r][1],
                    text="urgent - " + str(support_levels[r][1]),
                    font=dict(size=15, color="red"),
                )
